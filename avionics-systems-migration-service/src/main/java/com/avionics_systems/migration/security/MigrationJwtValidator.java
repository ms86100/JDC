...
/**
 * Validates platform JWT (same secret as avionics-systems-auth-service / gateway).
 */
@Component
public class MigrationJwtValidator {
    ...
    public MigrationJwtValidator(@Value("${jwt.secret:avionics-systems-platform-super-secret-key-that-is-at-least-256-bits-long}") String secret) {
    ...
    public static String resolveMigrationRole(Set<String> platformRoles, String headerMigrationRole) {
        if (platformRoles != null) {
            if (platformRoles.contains("ADMIN")
                    || platformRoles.contains("ROLE_ADMIN")
                    || platformRoles.contains("MIGRATION_ADMIN")) {
                return "MIGRATION_ADMIN";
            }
            if (platformRoles.contains("PROJECT_ADMIN")
                    || platformRoles.contains("ROLE_PROJECT_ADMIN")
                    || platformRoles.contains("MIGRATION_OPERATOR")) {
