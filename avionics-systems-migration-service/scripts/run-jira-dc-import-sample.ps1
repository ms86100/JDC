# Run Legacy DC sample import (Comment + Attachment with base64 file field)
param(
...
Write-Host "POST $BaseUrl/api/migration/import/legacy-dc"
Write-Host "Sample: $SampleXml"
Write-Host "Options: $optionsJson"

try {
    $response = Invoke-RestMethod -Uri "$BaseUrl/api/migration/import/legacy-dc" -Method Post -Headers $headers -Body $bodyLines
    $jobId = $response.id
    ...
    Write-Error $_
    Write-Host "`nTip: Start migration-service on 8094, or validate parse only with: mvn test -Dtest=LegacyDcXmlParserTest"
    exit 1
}
