...
import com.avionics_systems.admin.entity.WorkflowEntity;
...
/**
 * Proxies admin workflow CRUD to avionics-systems-workflow-service (canonical jira_workflow schema).
...
public class WorkflowAdminProxyService {
    ...
    public List<Map<String, Object>> getConditionDefinitions() {
        return fetchDefinitionList("/api/admin/workflows/conditions/definitions");
    }

    public List<Map<String, Object>> getValidatorDefinitions() {
        return fetchDefinitionList("/api/admin/workflows/validators/definitions");
    }

    public List<Map<String, Object>> getPostFunctionDefinitions() {
        return fetchDefinitionList("/api/admin/workflows/post-functions/definitions");
    }

    public List<Map<String, Object>> listScreens(String screenType) {
        String path = "/api/admin/workflows/screens";
        if (screenType != null && !screenType.isEmpty()) {
            path = path + "?screenType=" + screenType;
        }
        return fetchDefinitionList(path);
    }

    private List<Map<String, Object>> fetchDefinitionList(String path) {
        try {
            JsonNode body = restTemplate.getForObject(workflowServiceUrl + path, JsonNode.class);
            if (body == null || !body.isArray()) {
                return List.of();
            }
            return objectMapper.convertValue(body, new TypeReference<>() {});
        } catch (Exception e) {
            log.warn("Failed to proxy {} to workflow service: {}", path, e.getMessage());
            return List.of();
        }
    }
