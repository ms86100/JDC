package com.avionics_systems.document.controller;

import com.avionics_systems.document.dto.*;
import com.avionics_systems.document.service.DocumentService;
...
public class DocumentController {
    ...
    public ResponseEntity<DocumentVersionResponse> createVersion(
            @Parameter(description = "Document ID") @PathVariable UUID id,
            @RequestBody com.avionics_systems.document.dto.CreateDocumentRequest request,
            @RequestHeader(value = "X-User-Id", required = false) UUID userId) {
        if (userId == null) { throw new IllegalArgumentException(messageSource.getMessage("error.header.user-id.required", null, Locale.ENGLISH)); }
        UUID actor = userId;
        return ResponseEntity.status(HttpStatus.CREATED)
