...
CREATE TABLE jira_search. jira_search.search_index (
...
CREATE INDEX jira_search. idx_search_vector ON jira_search.search_index USING GIN (search_vector);

-- Create B-tree indexes for common lookups
CREATE INDEX jira_search. idx_entity_type ON jira_search.search_index (entity_type);
CREATE INDEX jira_search. idx_entity_id ON jira_search.search_index (entity_id);
CREATE INDEX jira_search. idx_entity_type_entity_id ON jira_search.search_index (entity_type, entity_id);

-- Create index for sorting and filtering by creation date
CREATE INDEX jira_search. idx_created_at ON jira_search.search_index (created_at DESC);

-- Create index for title searches (prefix matching)
CREATE INDEX jira_search. idx_title ON jira_search.search_index (LOWER(title));

-- Partial index for active issues (optimizes common queries)
CREATE INDEX jira_search. idx_active_issues ON jira_search.search_index (entity_type, entity_id)
...
CREATE TABLE jira_search. IF NOT EXISTS jira_search.search_query_log (
...
CREATE INDEX jira_search. idx_query_log_created_at ON jira_search.search_query_log (created_at DESC);
CREATE INDEX jira_search. idx_query_log_hash ON jira_search.search_query_log (query_hash);

-- Create table for tracking reindex operations
CREATE TABLE jira_search. IF NOT EXISTS jira_search.reindex_history (
    id SERIAL PRIMARY KEY,
    entity_type VARCHAR(100),
