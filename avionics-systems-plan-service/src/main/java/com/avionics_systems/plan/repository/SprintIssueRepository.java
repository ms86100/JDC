...
@Repository
public interface SprintIssueRepository extends JpaRepository<SprintIssue, UUID>, JpaSpecificationExecutor<SprintIssue> {
    ...
    @Query("SELECT si FROM SprintIssue si WHERE si.planItem.id = :planItemId AND si.removedAt IS NULL")
    List<SprintIssue> findByPlanItemIdAndRemovedAtIsNull(@Param("planItemId") UUID planItemId);

    @Query("SELECT si FROM SprintIssue si WHERE si.sprint.id = :sprintId AND si.completionStatus != 'DROPPED'")
    Page<SprintIssue> findActiveBySprintIdPaginated(@Param("sprintId") UUID sprintId, Pageable pageable);
}