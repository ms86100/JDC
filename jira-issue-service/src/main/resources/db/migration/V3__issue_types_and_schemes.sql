...
DO $$
BEGIN
    -- Some environments already have the enhanced issue_types schema (e.g. issue_type_key is NOT NULL).
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'jira_issue'
          AND table_name = 'issue_types'
          AND column_name = 'issue_type_key'
    ) THEN
        -- Prefer the richer insert when supported.
        IF EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'jira_issue'
              AND table_name = 'issue_types'
              AND column_name = 'is_subtask'
        ) THEN
            INSERT INTO jira_issue.issue_types (id, name, issue_type_key, icon, description, is_subtask)
            VALUES ('00000000-0000-0001-0001-000000000005', 'Sub-task', 'sub-task', 'subtask', 'A sub-task of a parent issue', TRUE)
            ON CONFLICT (name) DO NOTHING;
        ELSE
            INSERT INTO jira_issue.issue_types (id, name, issue_type_key, icon, description)
            VALUES ('00000000-0000-0001-0001-000000000005', 'Sub-task', 'sub-task', 'subtask', 'A sub-task of a parent issue')
            ON CONFLICT (name) DO NOTHING;
        END IF;
    ELSE
        -- Fallback for the original schema.
        INSERT INTO jira_issue.issue_types (id, name, icon, description)
        VALUES ('00000000-0000-0001-0001-000000000005', 'Sub-task', 'subtask', 'A sub-task of a parent issue')
        ON CONFLICT (name) DO NOTHING;
    END IF;
END $$;
