...
import com.avionics_systems.workflow.repository.WorkflowEventOutboxRepository;
import com.avionics_systems.workflow.service.ScriptListenerService;
...
public class WorkflowEventOutboxProcessor {
    ...
    @Value("${avionics-systems.workflow.outbox.batch-size:50}")
    private int batchSize;

    @Value("${app.workflow.outbox.default-transition-name:Transition}")
    private String defaultTransitionName;

    @Scheduled(fixedDelayString = "${avionics-systems.workflow.outbox.poll-interval-ms:5000}")
