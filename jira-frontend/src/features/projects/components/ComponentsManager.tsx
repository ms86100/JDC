  ...
  if (isError) {
    return (
      <ProjectLoadError
        ...
        message="Start jira-component-service (port 8097) and restart the gateway, then retry."
        onRetry={() => refetch()}
      />
    );
  }
    ...
    <>
      ...
      {filtered.length === 0 ? (
        <div className="sa-project-subpage-empty">
          <p>
            {components.length === 0
              ? 'No components yet. Components group issues and define default assignees (Jira DC parity).'
