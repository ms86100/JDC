...
export default function MigrationVerificationPanel({ jobId }: Props) {
  ...
  const isStillRunning = rawStatus === 'PENDING' || (data?.checks || []).some((c) => c.includes('IN_PROGRESS'));
  const status = isStillRunning ? 'RUNNING' : rawStatus;
  const statusColor =
      ...
      {isLoading ? (
        ...
        <>
          <p className="text-sm font-medium mb-2">
            Overall: {status === 'RUNNING' ? 'Verifying...' : status}
          </p>
          <ul className="text-sm space-y-1">
            {(data?.checks || []).filter((c) => !isStillRunning || !c.includes('FAIL')).map((c, i) => (
              <li key={i}>{c}</li>
            ))}
          </ul>
          {data && (
