...
export default function MigrationFieldProvisionPanel({
  ...
  return (
    ...
    >
      <div>
        <h3 className="font-semibold text-blue-900">Jira DC field provisioning</h3>
        <p className="text-sm text-blue-800 mt-1">
          Like Jira Data Center <strong>External System Import</strong>, unmapped CSV columns can be auto-created as
      ...
      {attachmentColumnPresent && (
        <div className="text-sm text-amber-900 bg-amber-50 border border-amber-200 rounded p-2">
          <strong>Attachments column detected.</strong> Map URLs (https://…) per Jira DC External Import, or use{' '}
          <strong>Jira DC XML + attachment bundle</strong> for full binary parity. Lightweight CSV import cannot attach
          files without URLs.
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2">
        <button
          type="button"
          className="px-3 py-1.5 text-sm bg-jira-blue text-white rounded hover:opacity-90 disabled:opacity-50"
