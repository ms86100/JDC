...
function rowToDraft(row: WizardUserMappingRow): UserMappingDraft {
  return {
    ...
    sourceType: row.sourceType ?? 'JIRA_DC',
...
export default function WizardUserMappingPanel({
  ...
  const addRow = () => {
    ...
    onChange([
      ...value,
      { sourceIdentifier: id, sourceType: 'JIRA_DC', mappingType: 'MANUAL' },
    ]);
    setNewSource('');
  };
    ...
    <div className="bg-white rounded-lg border p-6 space-y-4" data-testid="wizard-user-mapping-panel">
      <div>
        ...
        <p className="text-sm text-gray-600 mt-1">
          Map Jira DC users (username or email) to target platform accounts before import.
      ...
      <div className="flex flex-wrap gap-2 items-end">
        ...
        {migrationJobId && (
          <button
            type="button"
            className="px-4 py-2 bg-jira-blue text-white rounded hover:opacity-90 disabled:opacity-50"
