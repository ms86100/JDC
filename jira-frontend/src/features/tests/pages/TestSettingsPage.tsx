...
const GeneralTab: React.FC<GeneralTabProps> = ({ projectId, onSettingsChange }) => {
  ...
  useEffect(() => {
    const settingsKey = `jira-test-settings-${projectId}`;
...
const IntegrationsTab: React.FC<{ projectId: string }> = ({ projectId }) => {
  const [integrations, setIntegrations] = useState({
    jira: { enabled: true, url: 'https://jira.example.com', apiKey: '****', autoLink: true },
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Link tests to Jira issues</p>
            </div>
          </div>
          <label className="flex items-center gap-2">
            <span className="text-sm text-gray-500">{integrations.jira.enabled ? 'Enabled' : 'Disabled'}</span>
            <input
              type="checkbox"
        ...
        {integrations.jira.enabled && (
          <div className="space-y-4 mt-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  onChange={(e) => setIntegrations({ ...integrations, jira: { ...integrations.jira, apiKey: e.target.value } })}
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={integrations.jira.autoLink}
                onChange={(e) => setIntegrations({ ...integrations, jira: { ...integrations.jira, autoLink: e.target.checked } })}
                className="w-4 h-4 rounded border-gray-300"
              />
              <span className="text-sm">Auto-link test results to Jira issues</span>
            </label>
          </div>
        )}
      </div>
...
export const TestSettingsPage: React.FC<{ projectId?: string }> = ({ projectId: propProjectId }) => {
  ...
  const saveMutation = useMutation({
    mutationFn: async (settings: Record<string, unknown>) => {
      ...
      const settingsKey = propProjectId ? `jira-test-settings-${propProjectId}` : 'jira-test-settings';
