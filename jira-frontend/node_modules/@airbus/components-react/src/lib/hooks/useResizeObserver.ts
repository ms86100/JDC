...
type Size = {
  ...
  /** The ResizeObserverEntry of the observed element. */
...
const useResizeObserver = <T extends HTMLElement = HTMLElement>(
  ...
  const [{ width, height, entry }, setSize] = useState<Size>(initialSize);
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      ...
      if (hasChanged) {
        const newSize: Size = { width: newWidth, height: newHeight, entry };
        previousSize.current.width = newWidth;
        previousSize.current.height = newHeight;
