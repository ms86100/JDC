# Jira Data Center XML sample (migration import)
...
2. **Entity backup** — `JiraDcBackup/Entity/entityName/field` (see below)

Native Atlassian Jira ZIP backup (`entities.xml`) is detected but **not yet imported**.

## Canonical test file

```
jira-migration-service/src/test/resources/samples/jira-dc-minimal-comment-attachment.xml
```
...
mvn -q test -Dtest=JiraDcXmlParserTest
```
...
Native Jira Server/Data Center backup exports use proprietary XML inside the backup archive. This project normalizes imports through the **Entity + field** contract above. When exporting from a custom DC plugin or ETL, map columns to the field names in `JiraDcEntityMapper`.
