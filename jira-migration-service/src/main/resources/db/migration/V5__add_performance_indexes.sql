...
ON jira_migration.entity_status(source_identifier, job_id);
...
ON jira_migration.custom_field_options(custom_field_id, disabled, display_order)
WHERE disabled = false;

-- Parent option hierarchy
CREATE INDEX IF NOT EXISTS idx_custom_field_options_parent
ON jira_migration.custom_field_options(parent_option_id, display_order)
...
WHERE schemaname = 'jira_migration'
ORDER BY idx_scan ASC;

DO $$
BEGIN
    RAISE NOTICE 'Performance indexes created successfully:';
    RAISE NOTICE '  - Migration jobs: 6 indexes';
    RAISE NOTICE '  - Entity status: 4 indexes';
    RAISE NOTICE '  - Field definitions: 5 indexes';
    RAISE NOTICE '  - Issue field values: 3 indexes';
    RAISE NOTICE '  - Custom field options: 2 indexes';
    RAISE NOTICE '  - Field mappings: 1 index';
END $$;