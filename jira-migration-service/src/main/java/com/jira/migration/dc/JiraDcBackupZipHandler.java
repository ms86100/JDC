...
/**
 * Extracts Jira DC backup ZIP archives (entities.xml + data/attachments).
 */
@Component
@Slf4j
public class JiraDcBackupZipHandler {
    ...
    public ExtractedBackup extractZipToTemp(Path zipFile) throws IOException {
        Path extractRoot = Files.createTempDirectory("jira-dc-backup-");
        long totalBytes = 0;
        int entries = 0;
    ...
    static Path safeResolve(Path root, String entryName) throws IOException {
        String sanitized = JiraDcAttachmentBundleResolver.sanitizeFileName(entryName.replace('\\', '/'));
