...
import com.jira.migration.service.clients.WorkflowServiceClient;
import com.jira.migration.service.clients.dto.ImportWorkflowDescriptorRequest;
import com.jira.migration.service.clients.dto.WorkflowResponse;
import com.jira.migration.workflow.graph.WorkflowGraph;
import com.jira.migration.workflow.model.*;
import com.jira.migration.workflow.registry.OsWorkflowDescriptorRegistry;
import com.jira.migration.workflow.support.WorkflowXmlStatusMapper;
    ...
    public WorkflowResponse pushToWorkflowService(WorkflowDescriptorModel descriptor, WorkflowGraph graph,
        ...
        for (WorkflowStepModel step : descriptor.getSteps()) {
            UUID platformId = statusMapper.resolvePlatformStatusId(
                    step.getMeta().get("jira.status.id"),
                    statusMapper.resolveStatusName(step.getMeta().get("jira.status.id"), step.getName(), step.getMeta()));
        ...
        ImportWorkflowDescriptorRequest request = ImportWorkflowDescriptorRequest.builder()
                ...
                .description(descriptor.getMeta().getOrDefault("jira.description", "Imported from Jira DC XML"))
