...
-- ============================================
INSERT INTO jira_issue.issue_link_types (name, inward, outward) VALUES
...
INSERT INTO jira_issue.resolutions (name, description, sort_order) VALUES
    ('Fixed', 'The issue has been fixed', 1),
    ('Won''t Fix', 'The issue will not be fixed', 2),
    ('Duplicate', 'The issue is a duplicate', 3),
    ('Incomplete', 'The issue cannot be completed', 4),
    ('Cannot Reproduce', 'The issue cannot be reproduced', 5),
    ('Done', 'The issue has been completed', 6)
ON CONFLICT (name) DO NOTHING;
...
-- The attachments table is (re)created later in V5; only enhance it here if it
-- already exists so this migration does not fail on a fresh schema.
-- ============================================
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables
        WHERE table_schema = 'jira_issue' AND table_name = 'attachments'
    ) THEN
...
-- watchers/votes/project_versions/project_components are created later in V5
-- (which also creates these indexes); only index here if the table already exists.
-- issue_links here (V2) uses destination_issue_id; the target_issue_id column only
-- appears after V5 recreates the table, so guard that index by column existence.
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema='jira_issue' AND table_name='issue_links' AND column_name='target_issue_id') THEN
        CREATE INDEX IF NOT EXISTS idx_issue_links_target ON jira_issue.issue_links(target_issue_id);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='jira_issue' AND table_name='watchers') THEN
        CREATE INDEX IF NOT EXISTS idx_watchers_user ON jira_issue.watchers(user_id);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='jira_issue' AND table_name='votes') THEN
        CREATE INDEX IF NOT EXISTS idx_votes_user ON jira_issue.votes(user_id);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='jira_issue' AND table_name='project_versions') THEN
        CREATE INDEX IF NOT EXISTS idx_versions_project ON jira_issue.project_versions(project_id);
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='jira_issue' AND table_name='project_components') THEN
...
-- Counter triggers depend on watchers/votes tables created in V5 (which also
-- defines equivalent counter triggers); only create them here if present.
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='jira_issue' AND table_name='watchers') THEN
...
-- The comments table is (re)created in V5 with these columns/index; only enhance
-- it here if it already exists so this migration succeeds on a fresh schema.
-- ============================================
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='jira_issue' AND table_name='comments') THEN
