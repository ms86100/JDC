...
INSERT INTO jira_issue.issue_types (id, name, icon, description) VALUES
    (gen_random_uuid(), 'Test', 'test', 'A test case with steps, requirements, and execution tracking')
...
CREATE INDEX IF NOT EXISTS idx_tenv_project ON jira_issue.test_environments(project_id);
CREATE INDEX IF NOT EXISTS idx_tenv_active ON jira_issue.test_environments(is_active);
...
-- Root folder seed removed - will be created when first project is set up
