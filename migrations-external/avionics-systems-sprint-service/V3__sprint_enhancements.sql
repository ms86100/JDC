...
RETURNS VOID AS $$
BEGIN
    INSERT INTO jira_sprint.sprint_burndown (sprint_id, snapshot_date, total_issues, completed_issues, total_story_points, completed_story_points)
    SELECT
        p_sprint_id,
        CURRENT_DATE,
        COUNT(*),
        COUNT(*) FILTER (WHERE i.status IN ('DONE', 'CLOSED', 'RESOLVED')),
        COALESCE(SUM(i.story_points), 0),
        COALESCE(SUM(i.story_points) FILTER (WHERE i.status IN ('DONE', 'CLOSED', 'RESOLVED')), 0)
