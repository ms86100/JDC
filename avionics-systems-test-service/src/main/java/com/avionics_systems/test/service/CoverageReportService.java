...
public class CoverageReportService {
    ...
    @Value("${app.defaults.coverage.minimum-diversity:3}")
    private int minimumDiversity;

    @Value("${app.defaults.coverage.compliance-default-days:90}")
    private int complianceDefaultDays;
    ...
    public CoverageExportResponse generateDetailedAnalysis(CoverageExportRequest request) {
        ...
        projectCoverage.getByRequirement().forEach((reqKey, reqCoverage) -> {
            ...
            String status = reqCoverage.getCoverage().compareTo(BigDecimal.valueOf(coveredThreshold)) >= 0 ? "COVERED" :
                    reqCoverage.getCoverage().compareTo(BigDecimal.valueOf(partialThreshold)) >= 0 ? "PARTIAL" : "UNCOVERED";
    ...
    public CoverageExportResponse generateComplianceReport(CoverageExportRequest request) {
        ...
        return CoverageExportResponse.builder()
                ...
                .reportType(request.getReportType())
                .format(request.getFormat())
                .periodStart(request.getStartDate() != null ? request.getStartDate() : LocalDateTime.now().minusDays(complianceDefaultDays))
    ...
    public CoverageExportResponse generateTestDiversityReport(CoverageExportRequest request) {
        ...
        // minimumDiversity configured via @Value
