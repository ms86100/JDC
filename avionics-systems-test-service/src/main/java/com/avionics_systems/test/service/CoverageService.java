...
public class CoverageService {
    ...
    @Value("${app.defaults.coverage.trend-improving-threshold:5}")
    private int trendImprovingThreshold;

    @Value("${app.defaults.coverage.trend-declining-threshold:-5}")
    private int trendDecliningThreshold;

    @Value("${app.defaults.coverage.covered-threshold:80}")
    private int coveredThreshold;

    @Value("${app.defaults.coverage.partial-threshold:50}")
    ...
    private String determineTrendDirection(BigDecimal changeRate) {
        if (changeRate.compareTo(BigDecimal.valueOf(trendImprovingThreshold)) > 0) return "IMPROVING";
        if (changeRate.compareTo(BigDecimal.valueOf(trendDecliningThreshold)) < 0) return "DECLINING";
        return "STABLE";
    }
    ...
    public CoverageMatrixResponse getCoverageMatrix(UUID projectId) {
        ...
        for (String reqKey : requirementKeys) {
            ...
            for (TestSet testSet : testSets) {
                ...
                String status = coverage.compareTo(BigDecimal.valueOf(coveredThreshold)) >= 0 ? "COVERED" :
                        coverage.compareTo(BigDecimal.valueOf(partialThreshold)) >= 0 ? "PARTIAL" : "UNCOVERED";
