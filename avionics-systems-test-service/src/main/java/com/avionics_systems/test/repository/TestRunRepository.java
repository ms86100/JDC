...
import com.avionics_systems.test.entity.TestRun;
...
public interface TestRunRepository extends JpaRepository<TestRun, UUID> {
    ...
    // Tag queries
    @Query(value = "SELECT * FROM test_run tr WHERE tr.project_id = :projectId AND tr.tags IS NOT NULL AND :tag = ANY(tr.tags) ORDER BY tr.executed_at DESC", nativeQuery = true)
    List<TestRun> findByProjectIdAndTag(@Param("projectId") UUID projectId, @Param("tag") String tag);

    @Query(value = "SELECT * FROM test_run tr WHERE tr.project_id = :projectId AND tr.tags IS NOT NULL AND tr.tags && CAST(:tags AS text[]) ORDER BY tr.executed_at DESC", nativeQuery = true)
