...
import com.avionics_systems.sprint.dto.CreateSprintRequest;
import com.avionics_systems.sprint.dto.SprintResponse;
import com.avionics_systems.sprint.dto.UpdateSprintRequest;
import com.avionics_systems.sprint.service.SprintService;
 ...
 * avionics-systems-plan-service which is the single source of truth for all sprint operations.
 *
 * Legacy endpoints are maintained for backward compatibility only.
 * Please update your integration to use avionics-systems-plan-service endpoints:
 *
 ...
 *
 * @deprecated Use avionics-systems-plan-service SprintController instead
 */
@RestController
@RequestMapping("/api/sprints")
@RequiredArgsConstructor
@Slf4j
@Tag(name = "Sprints (DEPRECATED)", description = "⚠️ DEPRECATED: Use avionics-systems-plan-service instead")
@Deprecated
public class SprintController {

    private static final String DEPRECATION_WARNING =
        "This endpoint is deprecated. Use avionics-systems-plan-service instead. See: /api/plans/sprints";
    ...
    @ApiResponses({
        ...
        @ApiResponse(responseCode = "410", description = "Deprecated - use avionics-systems-plan-service")
    })
    ...
    @Operation(summary = "Get active sprint (DEPRECATED)",
               description = "⚠️ DEPRECATED. Use avionics-systems-plan-service instead")
    public ResponseEntity<SprintResponse> getActiveSprint(
            @RequestParam UUID projectId) {
        log.warn("DEPRECATED: /api/sprints/active GET called. Use avionics-systems-plan-service");
    ...
    private <T> ResponseEntity<T> deprecatedResponse(T body, HttpStatus status) {
        ...
        headers.add("Deprecation", "true");
        headers.add("Sunset", "2026-12-31");
        headers.add("X-Suggested-Route", "Use avionics-systems-plan-service /api/plans/* endpoints");
    ...
    private <T> ResponseEntity<List<T>> deprecatedListResponse(List<T> body) {
        ...
        headers.add("Deprecation", "true");
        headers.add("Sunset", "2026-12-31");
        headers.add("X-Suggested-Route", "Use avionics-systems-plan-service /api/plans/boards/* endpoints");
        headers.add("Warning", DEPRECATION_WARNING);
