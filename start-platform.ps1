...
function Start-ServiceFast {
    ...
    try {
        if ($Service.type -eq "backend" -or $Service.type -eq "gateway") {
            ...
            # Build direct Spring datasource overrides from service environment
            $dbHost = if ($Service.environment.DB_HOST) { $Service.environment.DB_HOST } else { "localhost" }
            $dbPort = if ($Service.environment.DB_PORT) { $Service.environment.DB_PORT } else { "5432" }
            $dbName = if ($Service.environment.DB_NAME) { $Service.environment.DB_NAME } else { "systems" }
            $dbUser = if ($Service.environment.DB_USERNAME) { $Service.environment.DB_USERNAME } else { "systems_admin" }
            $dbPass = if ($Service.environment.DB_PASSWORD) { $Service.environment.DB_PASSWORD } else { "" }
            $dbUrl  = "jdbc:postgresql://${dbHost}:${dbPort}/${dbName}"

            $argList = @(
                "-Xms$heapSize", "-Xmx$heapSize",
                "-Dspring.datasource.url=$dbUrl",
                "-Dspring.datasource.username=$dbUser",
                "-Dspring.datasource.password=$dbPass",
                "-Dspring.flyway.url=$dbUrl",
                "-Dspring.flyway.user=$dbUser",
                "-Dspring.flyway.password=$dbPass",
                "-Dspring.flyway.validate-on-migrate=false",
                "-Djira.permissions.fail-open=true",
                "-Dspring.jpa.hibernate.ddl-auto=none",
                "-Dmanagement.health.redis.enabled=false",
                "-Dmigration.security.enabled=false",
                "-Dspring.datasource.hikari.maximum-pool-size=5",
                "-Dspring.datasource.hikari.minimum-idle=1",
                "-jar", $jarPath, "--server.port=$($Service.port)"
            )
