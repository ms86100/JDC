...
function renderSysdopsRelations() {
  return (
    <div>
      ...
      <Paragraph>
        SYSDOPS (System DevOps) is an Airbus Avionics Systems platform managing the full aircraft
        ...
        Avionics Systems issue links to form a complete traceability chain from design requirements through
...
function renderSystemOverview() {
  return (
    <div>
      ...
      <Paragraph>
        SYSDOPS (System DevOps) is the Avionics Systems platform for managing the full Airbus V&V
      ...
      <Paragraph>
        The platform is a clustered, enterprise-grade microservices system built on Avionics Systems
...
`  ┌──────────────┐    REST (write)     ┌──────────────┐
  ...
  Topics: avisys.issue.events | avisys.workflow.events |
          avisys.notification.events | avisys.audit.events |
          avisys.search.index | avisys.user.events | avisys.project.events`
        }</div>
      </div>
...
function renderClusterArchitecture() {
  const clusterLayers = [
    ...
    { layer: 'Shared State', desc: 'Single source of truth shared by all nodes', color: C.success, items: ['PostgreSQL (14 schemas)', 'Redis (cache, pub/sub, locking)', 'MinIO (S3-compatible storage)'] },
  ];
  ...
  const components = [
    { name: 'DistributedLockService', pkg: 'com.avisys.cluster.lock', desc: 'Database or Redis-backed distributed locking' },
    { name: 'ShedLockAutoConfiguration', pkg: 'com.avisys.cluster.scheduler', desc: 'JDBC-based scheduler lock provider' },
    { name: 'StorageProvider', pkg: 'com.avisys.cluster.storage', desc: 'Local / S3 (MinIO) file storage abstraction' },
    { name: 'ClusterCacheManager', pkg: 'com.avisys.cluster.cache', desc: 'Caffeine L1 + Redis L2 with cross-node invalidation' },
    { name: 'ClusterEventBus', pkg: 'com.avisys.cluster.event', desc: 'Redis pub/sub for real-time cross-node messaging' },
  ];
  ...
  return (
    <div>
      ...
      <Paragraph>
        The platform supports Avionics Systems-style clustering: multiple identical application nodes behind a load
        balancer, sharing a database and object storage. Each service can be independently scaled — Docker DNS
        automatically round-robins requests across all instances. The avisys-cluster-commons library provides
      ...
      <div style={{ position: 'relative', padding: 20, background: C.bg, borderRadius: 8, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 16 }}>
          <DiagramBox label="PostgreSQL" sub="14 schemas" color={C.dbBg} width={180} />
      ...
      <div className="ads-card" style={{ marginTop: 8 }}>
        <h4 className="ads-card-title">avisys-cluster-commons Library</h4>
        <Paragraph>
...
function renderEnterpriseHardening() {
  ...
  return (
    <div>
      ...
      <div className="ads-card" style={{ marginTop: 16 }}>
        <h4 className="ads-card-title">Cluster Commons Library (avisys-cluster-commons)</h4>
        <Paragraph>
...
function renderPlatformCapabilities() {
  const phases = [
    ...
    },
    {
      ...
      items: [
        'Prometheus scraping all 22 services at /actuator/prometheus',
    ...
    <div>
      ...
      <div style={{ position: 'relative', padding: 20, background: C.bg, borderRadius: 8, marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 8 }}>
          <DiagramBox label="Prometheus" sub="Metrics (22 services)" color={C.warning} width={160} />
...
function renderServiceMap() {
  const services = [
    { name: 'auth-service', port: '8081', schema: 'avisys_auth', entities: 4, keyModels: 'Role, UserGroup, Permission, SessionToken', dependsOn: '--', keyEndpoints: '/api/auth/login, /api/auth/register, /api/auth/token/refresh' },
    { name: 'user-service', port: '8082', schema: 'avisys_admin', entities: 3, keyModels: 'CwdUser, CwdGroup, CwdMembership', dependsOn: 'auth', keyEndpoints: '/api/users, /api/groups' },
    { name: 'project-service', port: '8083', schema: 'avisys_project', entities: 15, keyModels: 'Project, ProjectScheme, ProjectRole, Component, Version', dependsOn: 'auth, user', keyEndpoints: '/api/projects, /api/projects/{id}/roles' },
    { name: 'issue-service', port: '8084', schema: 'avisys_issue', entities: 48, keyModels: 'Issue, CustomField, ChangeCard, DesignItem, DCL, SystemStandard, Deliverable', dependsOn: 'project, workflow', keyEndpoints: '/api/issues, /api/issues/{id}/change-card, /api/issues/{id}/design-item' },
    { name: 'workflow-service', port: '8085', schema: 'avisys_workflow', entities: 31, keyModels: 'Workflow, Transition, Condition, Validator, PostFunction, AutomationRule, ScriptDefinition', dependsOn: 'issue, project', keyEndpoints: '/api/workflows, /api/automation/rules, /api/scripts' },
    { name: 'test-service', port: '8095', schema: 'avisys_test', entities: 80, keyModels: 'VvoDefinition, HlvvoDefinition, TestIssue, TestPlan, TestExecution, TechEvent, BenchDefect, ProblemReport', dependsOn: 'admin, workflow', keyEndpoints: '/api/vvo, /api/tech-events, /api/vv-reports, /api/export-templates' },
    { name: 'admin-service', port: '8093', schema: 'avisys_admin', entities: 58, keyModels: 'MasterDataCategory, MasterDataValue, Asset, AssetType, IssueType, Status, Priority', dependsOn: '--', keyEndpoints: '/api/admin/master-data, /api/admin/assets, /api/admin/issue-types' },
    { name: 'search-service', port: '8088', schema: 'avisys_search', entities: 4, keyModels: 'JQLQuery, JQLClause, SearchIndex, SavedFilter', dependsOn: 'issue', keyEndpoints: '/api/search, /api/jql/parse, /api/jql/autocomplete' },
    { name: 'sprint-service', port: '8091', schema: 'avisys_sprint', entities: 5, keyModels: 'Sprint, Board, BoardConfig, SprintIssue, BoardColumn', dependsOn: 'issue', keyEndpoints: '/api/sprints, /api/boards' },
    { name: 'plan-service', port: '--', schema: 'avisys_plan', entities: 10, keyModels: 'PlanItem, PlanTeam, PlanGoal, PlanDependency, PlanRelease', dependsOn: 'project', keyEndpoints: '/api/plans, /api/plans/{id}/goals, /api/plans/{id}/teams' },
    { name: 'notification-service', port: '8087', schema: 'avisys_notification', entities: 3, keyModels: 'EmailTemplate, NotificationEvent, NotificationPreference', dependsOn: 'user', keyEndpoints: '/api/notifications, /api/notification-preferences' },
    ...
    { name: 'comment-service', port: '--', schema: 'avisys_comment', entities: 1, keyModels: 'Comment', dependsOn: 'issue', keyEndpoints: '/api/comments' },
    { name: 'attachment-service', port: '--', schema: '--', entities: 2, keyModels: 'Attachment, AttachmentMetadata', dependsOn: 'issue', keyEndpoints: '/api/attachments' },
    { name: 'version-service', port: '--', schema: 'avisys_version', entities: 2, keyModels: 'ProjectVersion, IssueAffectsVersion', dependsOn: 'project', keyEndpoints: '/api/versions' },
    { name: 'component-service', port: '--', schema: 'avisys_component', entities: 5, keyModels: 'ProjectComponent, IssueComponent, ComponentAssignmentRule', dependsOn: 'project', keyEndpoints: '/api/components' },
    { name: 'migration-service', port: '--', schema: '--', entities: 8, keyModels: 'DcStagingEntry, WorkflowDescriptorModel, ImportWizard', dependsOn: 'issue, workflow', keyEndpoints: '/api/migration, /api/import-wizard' },
    { name: 'audit-service', port: '--', schema: 'avisys_audit', entities: 1, keyModels: 'AuditLog', dependsOn: '--', keyEndpoints: '/api/audit' },
    { name: 'document-service', port: '--', schema: '--', entities: 3, keyModels: 'Document, DocumentTemplate, DocumentSection', dependsOn: 'issue', keyEndpoints: '/api/documents' },
    { name: 'report-service', port: '--', schema: '--', entities: 3, keyModels: 'ReportDefinition, ReportSchedule, ReportExecution', dependsOn: 'search', keyEndpoints: '/api/reports' },
    { name: 'portal-service', port: '--', schema: '--', entities: 2, keyModels: 'PortalConfig, PortalRequest', dependsOn: 'project', keyEndpoints: '/api/portal' },
  ];
        ...
        <div style={{ padding: 16, background: C.bg, borderRadius: 8, overflowX: 'auto' }}>
          <svg width="820" height="360" viewBox="0 0 820 360" style={{ display: 'block', margin: '0 auto' }}>
            <defs>
              ...
              <marker id="depArrowBrand" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
                <path d="M0,0 L8,3 L0,6" fill={C.brand} />
              </marker>
            </defs>

            {/* Nodes */}
            {[
              { id: 'auth', x: 340, y: 20, c: C.purple },
              { id: 'user', x: 180, y: 100, c: C.purple },
              { id: 'project', x: 60, y: 180, c: C.success },
              { id: 'issue', x: 280, y: 180, c: C.success },
              { id: 'workflow', x: 480, y: 180, c: C.danger },
              { id: 'admin', x: 660, y: 100, c: C.warning },
              { id: 'test', x: 480, y: 270, c: C.danger },
              { id: 'search', x: 60, y: 270, c: C.teal },
              { id: 'sprint', x: 280, y: 270, c: C.teal },
              { id: 'plan', x: 60, y: 330, c: C.teal },
              { id: 'notification', x: 660, y: 270, c: C.subtle },
              { id: 'dashboard', x: 660, y: 180, c: C.subtle },
            ].map((n) => (
              <g key={n.id}>
                <rect x={n.x} y={n.y} width={110} height={34} rx={5} fill={n.c} />
                <text x={n.x + 55} y={n.y + 21} textAnchor="middle" fill={C.white} fontSize={11} fontWeight={600}>
                  {n.id}
                </text>
              </g>
            ))}

            {/* Edges with arrowheads and labels */}
            {[
              { x1: 390, y1: 54, x2: 246, y2: 100, label: '' },
              { x1: 235, y1: 134, x2: 130, y2: 180, label: '' },
              { x1: 360, y1: 54, x2: 130, y2: 180, label: '' },
              { x1: 395, y1: 54, x2: 335, y2: 180, label: '' },
              { x1: 170, y1: 196, x2: 280, y2: 196, label: 'REST' },
              { x1: 390, y1: 196, x2: 480, y2: 196, label: 'REST' },
              { x1: 535, y1: 214, x2: 535, y2: 270, label: '' },
              { x1: 710, y1: 134, x2: 556, y2: 270, label: '' },
              { x1: 300, y1: 214, x2: 130, y2: 270, label: '' },
              { x1: 335, y1: 214, x2: 335, y2: 270, label: '' },
              { x1: 115, y1: 214, x2: 115, y2: 330, label: '' },
              { x1: 248, y1: 134, x2: 700, y2: 270, label: '' },
              { x1: 170, y1: 287, x2: 660, y2: 197, label: '' },
            ].map((e, i) => (
              <g key={i}>
                <line x1={e.x1} y1={e.y1} x2={e.x2} y2={e.y2} stroke={C.subtle} strokeWidth={1.5} markerEnd="url(#depArrow)" />
                {e.label && (
                  <text x={(e.x1 + e.x2) / 2} y={(e.y1 + e.y2) / 2 - 6} textAnchor="middle" fontSize={9} fill={C.brand} fontWeight={600}>
                    {e.label}
                  </text>
                )}
              </g>
            ...
            <g transform="translate(10, 10)">
              <rect x="0" y="0" width="8" height="8" rx="2" fill={C.purple} />
              <text x="12" y="8" fontSize="9" fill={C.subtle}>Identity</text>
              <rect x="55" y="0" width="8" height="8" rx="2" fill={C.success} />
              <text x="67" y="8" fontSize="9" fill={C.subtle}>Core</text>
              <rect x="95" y="0" width="8" height="8" rx="2" fill={C.danger} />
              <text x="107" y="8" fontSize="9" fill={C.subtle}>Engine</text>
              <rect x="145" y="0" width="8" height="8" rx="2" fill={C.teal} />
              <text x="157" y="8" fontSize="9" fill={C.subtle}>Agile</text>
            </g>
          </svg>
        </div>
      </div>
...
function renderIssueWorkflow() {
  ...
  const migrationFeatures = [
    { area: 'Data Center Import', entities: 'DcStagingEntry, DcUnknownCustomField — full Avionics Systems XML/JSON import', detail: 'Imports projects, issues, workflows, schemes, users, groups, custom fields from a Avionics Systems backup. Staged for review before commit.' },
    { area: 'Workflow XML Import', entities: 'WorkflowDescriptorModel, WorkflowStepModel, WorkflowActionModel, WorkflowFunctionDescriptor', detail: 'Import Avionics Systems workflow XML descriptors with full step/action/function/condition/validator mapping.' },
  ...
  return (
    <div>
      ...
      <p style={{ color: C.subtle, marginBottom: '16px' }}>
        The issue-service is the central data store for all Avionics Systems issues with 48+ entities. It manages the full issue lifecycle including custom fields, linking, versioning, time tracking, and integrates with the workflow engine for status transitions.
      </p>
      ...
      <div className="ads-card" style={{ marginBottom: '24px' }}>
        ...
        <div style={{ padding: '16px', background: C.bg, borderRadius: '8px' }}>
          <svg viewBox="0 0 900 400" style={{ width: '100%', maxHeight: '400px' }}>
            ...
            <defs>
              <marker id="ahDash" markerWidth="8" markerHeight="6" refX="3" refY="0" orient="auto">
                <path d="M0,0 L3,6 L6,0" fill={C.subtle} />
              </marker>
            </defs>
            <path d="M145,370 L145,392 L760,392 L760,370" fill="none" stroke={C.subtle} strokeWidth="1.5" strokeDasharray="4,4" markerEnd="url(#ahDash)"/>
            <text x="450" y="388" textAnchor="middle" fontSize="9" fill={C.subtle}>import issues, fields, schemes</text>
          </svg>
        </div>
      </div>
      ...
      <div className="ads-card">
        <div className="ads-card-title">Migration Service — Avionics Systems Data Import Architecture</div>
        <div className="ads-table-wrap">
          <table className="ads-table">
...
function renderDomainModel() {
  return (
    <div>
      ...
      <Paragraph>
        The SYSDOPS domain model maps the Airbus V&V lifecycle onto a Avionics Systems-style issue hierarchy. Four object
        categories (Requirement, Test, Change, Defect) are linked via typed Avionics Systems issue links to form a
        ...
        <div style={{ minWidth: 700, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
          ...
          <div style={{ marginTop: 24, borderTop: `2px dashed ${C.border}`, paddingTop: 20, width: '100%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: 24, flexWrap: 'wrap' }}>
              <DiagramBox label="ChangeCard" sub="7-tab master form" color={C.purple} width={140} />
              ...
              <DiagramBox label="DCL" sub="Design Clarification Log" color={C.purple} width={140} />
            </div>
      ...
      <div className="ads-card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ padding: 16, background: C.bg, borderRadius: 8, overflowX: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 700 }}>
            ...
            <DiagramBox label="ChangeCard" sub="7-tab form" color={C.purple} width={120} height={50} />
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <Paragraph>
          A baseline is a snapshot of official VVOs at a point in time, using Avionics Systems Fix Version as the
...
function renderPluginMatrix() {
  const plugins = [
    { feature: 'Test Management', plugin: 'Xray for Avionics Systems', impl: 'TestIssue, TestPlan, TestExecution, TestSet', svc: 'test-service', status: 'Full' },
    ...
    { feature: 'Automation Rules', plugin: 'Automation for Avionics Systems', impl: 'AutomationRule, AutomationEventListener, AutomationAction', svc: 'workflow-service', status: 'Full' },
    { feature: 'Document Export', plugin: 'XPorter for Avionics Systems', impl: 'ExportTemplate, DocumentExportService, section/field mapping', svc: 'test-service', status: 'Full' },
    { feature: 'Custom Charts', plugin: 'Custom Charts for Avionics Systems', impl: 'GadgetInstance (chartType, chartConfig JSON)', svc: 'dashboard-service', status: 'Full' },
    ...
    { feature: 'Change Management', plugin: 'Native Avionics Systems', impl: 'ChangeCard, DesignItem, DCL, Deliverable, Modification', svc: 'issue-service', status: 'Full' },
    ...
    { feature: 'Workflow Designer', plugin: 'Native Avionics Systems', impl: 'Workflow, Transition, Condition, Validator, PostFunction', svc: 'workflow-service', status: 'Full' },
    { feature: 'JQL Search', plugin: 'Native Avionics Systems', impl: 'JQLParser, JQLClause, SavedFilter', svc: 'search-service', status: 'Full' },
    { feature: 'Sprint Planning', plugin: 'Native Avionics Systems', impl: 'Sprint, Board, BoardConfig, BoardColumn', svc: 'sprint-service', status: 'Full' },
  ];
  ...
  return (
    <div>
      ...
      <Paragraph>
        SYSDOPS replaces 9+ Avionics Systems marketplace plugins with native implementations. Each
        ...
        <div className="ads-table-wrap">
          <table className="ads-table">
            ...
            <tbody>
              ...
              <tr><td style={{fontWeight:600}}>Statement Limit</td><td>ResourceLimits.statementLimit(500,000) — prevents infinite loops</td><td>avisys.scripting.max-statements</td></tr>
              <tr><td style={{fontWeight:600}}>Timeout</td><td>5s workflow / 10s console — context closed via ScheduledExecutorService</td><td>avisys.scripting.timeout-ms</td></tr>
              <tr><td style={{fontWeight:600}}>Memory</td><td>Best-effort monitoring — logs warning if growth exceeds memoryLimitMb</td><td>avisys.scripting.memory-limit-mb (64MB)</td></tr>
              <tr><td style={{fontWeight:600}}>HTTP Whitelist</td><td>Domain whitelist + SSRF protection (loopback/private IP blocking via DNS resolution)</td><td>avisys.scripting.http-whitelist-domains</td></tr>
              <tr><td style={{fontWeight:600}}>SQL Read-Only</td><td>Only SELECT/WITH allowed by default, INSERT/UPDATE/DELETE blocked</td><td>avisys.scripting.sql-write-enabled</td></tr>
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <div className="ads-table-wrap">
          <table className="ads-table">
            ...
            <tbody>
              {[
                ['avisys.scripting.enabled', 'true', 'Master switch — disables all script execution when false'],
                ['avisys.scripting.timeout-ms', '5000', 'Max execution time for workflow scripts (ms)'],
                ['avisys.scripting.console-timeout-ms', '10000', 'Max execution time for console/REPL scripts (ms)'],
                ['avisys.scripting.memory-limit-mb', '64', 'Memory monitoring threshold — logs warning if exceeded'],
                ['avisys.scripting.max-statements', '500000', 'GraalVM statement limit — prevents infinite loops'],
                ['avisys.scripting.log-retention-days', '30', 'Days to keep execution logs before ScriptLogCleanupJob deletes (runs daily at 2 AM)'],
                ['avisys.scripting.http-whitelist-domains', '[]', 'Allowed domains for http.get/post/put/delete — empty blocks all HTTP'],
                ['avisys.scripting.http-timeout-ms', '5000', 'Timeout for outbound HTTP calls from scripts'],
                ['avisys.scripting.env-whitelist-keys', '[]', 'Environment variables accessible via env.get() — empty blocks all'],
                ['avisys.scripting.scheduled-enabled', 'false', 'Enable scheduled script execution (cron-based)'],
                ['avisys.scripting.scheduled-poll-interval-ms', '30000', 'Polling interval for due schedules (ShedLock protected)'],
                ['avisys.scripting.sql-write-enabled', 'false', 'Allow INSERT/UPDATE/DELETE via sql.update() — false = read-only'],
                ['avisys.scripting.datasources', '{}', 'Named SQL datasources: {name: {url, username, password, driverClassName}}'],
              ].map(([prop, def, desc], i) => (
                <tr key={i}>
...
function renderAuthUsers() {
  ...
  const adminFeatures = [
    ...
    { area: 'Permission Schemes', detail: '32 standard Avionics Systems permissions seeded (BROWSE_PROJECTS, CREATE_ISSUES, EDIT_ISSUES, DELETE_ISSUES, ASSIGN_ISSUES, etc.)' },
    ...
    <div>
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ padding: 20, background: C.bg, borderRadius: 8, overflowX: 'auto' }}>
          <svg viewBox="0 0 920 260" style={{ width: '100%', maxHeight: 260 }}>
            <defs>
              <marker id="authArrowR" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
                <path d="M0,0 L8,3 L0,6" fill={C.subtle} />
              </marker>
              <marker id="authArrowD" markerWidth="8" markerHeight="6" refX="3" refY="6" orient="auto">
                <path d="M0,0 L3,6 L6,0" fill={C.subtle} />
              </marker>
              <marker id="authArrowDanger" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
                <path d="M0,0 L8,3 L0,6" fill={C.danger} />
              </marker>
            </defs>
            ...
            {/* Arrow */}
            <line x1="130" y1="55" x2="168" y2="55" stroke={C.subtle} strokeWidth="2" markerEnd="url(#authArrowR)"/>
            ...
            {/* Arrow */}
            <line x1="310" y1="55" x2="348" y2="55" stroke={C.subtle} strokeWidth="2" markerEnd="url(#authArrowR)"/>
            ...
            {/* Arrow */}
            <line x1="480" y1="55" x2="518" y2="55" stroke={C.subtle} strokeWidth="2" markerEnd="url(#authArrowR)"/>
            ...
            {/* Arrow */}
            <line x1="650" y1="55" x2="688" y2="55" stroke={C.subtle} strokeWidth="2" markerEnd="url(#authArrowR)"/>
            ...
            <line x1="755" y1="90" x2="755" y2="128" stroke={C.subtle} strokeWidth="2" markerEnd="url(#authArrowD)"/>
            ...
            <line x1="690" y1="165" x2="482" y2="165" stroke={C.danger} strokeWidth="1.5" strokeDasharray="4 3" markerEnd="url(#authArrowDanger)"/>

            {/* Refresh arrow */}
            <line x1="415" y1="190" x2="415" y2="218" stroke={C.subtle} strokeWidth="2" markerEnd="url(#authArrowD)"/>
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <Paragraph>
          ...
          avisys_admin schema with user-service.
        </Paragraph>
        <div className="ads-table-wrap">
          <table className="ads-table">
            <thead>
  ...
  return (
    <div>
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ padding: 20, background: C.bg, borderRadius: 8, overflowX: 'auto' }}>
          <svg viewBox="0 0 880 350" style={{ width: '100%', maxHeight: 350 }}>
            <defs>
              <marker id="sprintArrowR" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
                <path d="M0,0 L8,3 L0,6" fill={C.subtle} />
              </marker>
              <marker id="sprintArrowD" markerWidth="8" markerHeight="6" refX="3" refY="6" orient="auto">
                <path d="M0,0 L3,6 L6,0" fill={C.subtle} />
              </marker>
            </defs>
            ...
            {/* Arrow to columns */}
            <line x1="230" y1="70" x2="278" y2="70" stroke={C.subtle} strokeWidth="2" markerEnd="url(#sprintArrowR)"/>
            ...
            <line x1="130" y1="120" x2="130" y2="158" stroke={C.subtle} strokeWidth="2" markerEnd="url(#sprintArrowD)"/>
            ...
            <line x1="370" y1="207" x2="398" y2="207" stroke={C.subtle} strokeWidth="2" markerEnd="url(#sprintArrowR)"/>
            ...
            <line x1="480" y1="207" x2="508" y2="207" stroke={C.subtle} strokeWidth="2" markerEnd="url(#sprintArrowR)"/>
            ...
            {/* Arrow board -> config */}
            <path d="M230,40 Q370,5 498,40" fill="none" stroke={C.subtle} strokeWidth="1.5" strokeDasharray="4 3" markerEnd="url(#sprintArrowR)"/>
            <text x="370" y="18" textAnchor="middle" fontSize="9" fill={C.subtle}>1:1</text>
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <Paragraph>
          The search service implements a complete JQL (Avionics Systems Query Language) parser that translates
      ...
      <div className="ads-card">
        ...
        <Paragraph>
          The plan service implements Avionics Systems Advanced Roadmaps functionality: cross-project planning with
...
function renderDatabaseSchema() {
  const schemas = [
    {
      schema: 'avisys_auth', service: 'auth-service', port: '8081',
    ...
    {
      schema: 'avisys_admin', service: 'admin-service + user-service', port: '8093/8082',
    ...
    {
      schema: 'avisys_project', service: 'project-service', port: '8083',
    ...
    {
      schema: 'avisys_issue', service: 'issue-service', port: '8084',
    ...
    {
      schema: 'avisys_workflow', service: 'workflow-service', port: '8085',
    ...
    {
      schema: 'avisys_test', service: 'test-service', port: '8095',
    ...
    {
      schema: 'avisys_sprint', service: 'sprint-service', port: '8091',
    ...
    {
      schema: 'avisys_plan', service: 'plan-service', port: '--',
    ...
    <div>
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', padding: '16px 0', height: 200, borderBottom: `1px solid ${C.border}` }}>
          {schemas
            .map((s) => ({ name: s.schema.replace('avisys_', ''), count: parseInt(s.tableCount) || 1, color: s.color }))
      ...
      <div className="ads-card" style={{ marginBottom: 24 }}>
        ...
        <div style={{ padding: 16, background: C.bg, borderRadius: 8, overflowX: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 700 }}>
            <DiagramBox label="issue-service" sub="avisys_issue schema" color={C.success} width={140} height={55} />
            <ArrowRight label="REST call" width={80} />
            <DiagramBox label="workflow-service" sub="avisys_workflow schema" color={C.danger} width={150} height={55} />
            <ArrowRight label="REST call" width={80} />
            <DiagramBox label="admin-service" sub="avisys_admin schema" color={C.warning} width={140} height={55} />
          </div>
      ...
      <div className="ads-card">
        ...
        <div className="ads-alert ads-alert--info" style={{ marginTop: 16 }}>
          ...
          exception is <code>avisys_admin</code>, which is shared between admin-service (configuration data)
