...
interface KanbanFilterStripProps {
  ...
  legacyDcLayout?: boolean;
}

/** Filters + search + swimlanes in one compact row (Avionics Systems board filter bar). */
    ...
    <>
      <div className="sa-kanban-filter-strip" role="toolbar" aria-label="Board filters">
        <div className="sa-kanban-filter-strip-left">
          ...
          {legacyDcLayout && assignees.length > 0 && onAssigneeFilterChange && (
        ...
        <div className="sa-kanban-filter-strip-right">
          {legacyDcLayout && epicOptions.length > 0 && onEpicFilterChange && (
