...
export default function WorkflowsPage() {
  ...
  const handleCloneWorkflow = async (id: string, newName: string) => {
    try {
      await workflowApi.clone(id, newName);
      await fetchWorkflows();
    } catch (err) {
  ...
  const handleCreateScheme = async () => {
    ...
    try {
      await workflowApi.createScheme(schemeForm);
      await fetchSchemes();
  ...
  const handleDeleteScheme = async (id: string) => {
    ...
    try {
      await workflowApi.deleteScheme(id);
      await fetchSchemes();
...
function SchemeDetailView({ scheme, workflows, onBack, onRefresh }: SchemeDetailViewProps) {
  ...
  const fetchSchemeMappings = async () => {
    setLoading(true);
    try {
      const res = await workflowApi.getScheme(scheme.id);
  ...
  const handleAssignWorkflow = async (issueTypeId: string, workflowId: string) => {
    try {
      await workflowApi.addSchemeMapping(scheme.id, { issueTypeId, workflowId });
