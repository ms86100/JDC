...
import { useAviSysGroups, useCreateAviSysGroup, useDeleteAviSysGroup } from '../hooks/useAdminApi';
import type { AviSysGroup } from '../hooks/useAdminApi';
import './AviSysGroupsBrowser.css';

export default function AviSysGroupsBrowser() {
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(0);
  const pageSize = 20;
  const [newGroupName, setNewGroupName] = useState('');

  const { data, isLoading, refetch } = useAviSysGroups({
        ...
        <div className="group-name-cell">
          ...
          {group.isSystem && (
            <span className="group-badges">
              ...
              <span className="badge badge-avisys">AVIONICS SYSTEMS</span>
            </span>
          )}
        </div>
      </td>
