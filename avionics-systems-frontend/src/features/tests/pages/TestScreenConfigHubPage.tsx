...
export default function TestScreenConfigHubPage() {
  ...
  if (projectId) {
    return (
      <div className="p-6">
        <div className="mb-4">
          <Link to="/tests/screen-config" className="text-sm text-avisys-blue hover:underline">
            ← All projects
          </Link>
        </div>
          ...
          {projects.map((p: { id: string; name: string; key?: string }) => (
            <li key={p.id}>
              <Link
                ...
                className="block px-4 py-3 hover:bg-gray-50 text-sm font-medium text-avisys-blue"
              >
                {p.name}
