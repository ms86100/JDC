...
import { isLegacyDcIssueImport } from '../utils/importTypeHelpers';
...
export default function MigrationJobDetailPanel({ jobId, onClose, importType, resultMetadata }: Props) {
  const isDc = isLegacyDcIssueImport(importType ?? undefined);
                  ...
                  <li key={entry.id} className="flex justify-between items-start gap-2 border-b pb-2">
                    ...
                    {entry.id && (
                      <button
                        ...
                        className="text-xs px-2 py-1 bg-avisys-blue text-white rounded"
                      >
