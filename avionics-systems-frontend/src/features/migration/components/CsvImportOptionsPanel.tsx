...
export default function CsvImportOptionsPanel({
  ...
  return (
    ...
    >
      <div>
        <h3 className="font-semibold text-gray-900">Systems DC CSV import profile</h3>
        <p className="text-sm text-gray-600 mt-1">
          Mirrors Systems Data Center: lightweight issue CSV vs External System Import (attachments, subtasks).
        </p>
      </div>

      <div className="grid gap-3 sm:grid-cols-2">
        <label
          className={`border rounded-lg p-3 cursor-pointer ${value.csvImportProfile === 'LIGHTWEIGHT' ? 'border-avisys-blue bg-blue-50' : ''}`}
        >
          <input
            type="radio"
        ...
        <label
          className={`border rounded-lg p-3 cursor-pointer ${value.csvImportProfile === 'EXTERNAL' ? 'border-avisys-blue bg-blue-50' : ''} ${!isAdmin ? 'opacity-60' : ''}`}
        >
          <input
            type="radio"
            name="csvImportProfile"
        ...
        <div className="space-y-3 border-t pt-3">
          ...
          <div>
            ...
            <input
              ...
              placeholder="C:\avisys-home\import\attachments"
            />
            <p className="text-xs text-gray-500 mt-1">
              Systems DC: <code>FILE:filename</code> resolves here. Also set server env{' '}
