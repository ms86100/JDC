...
import { migrationApi, type LegacyDcValidateResponse } from '../../../api/serviceApi';
...
interface Props {
  ...
  initialResult?: LegacyDcValidateResponse | null;
  onValidated?: (result: LegacyDcValidateResponse) => void;
}
...
export default function DcImportValidationPanel({
  ...
  const [result, setResult] = useState<LegacyDcValidateResponse | null>(initialResult ?? null);
  ...
  const runValidate = async () => {
    ...
    setError(null);
    try {
      const response = await migrationApi.validateLegacyDcImport({
        file: xmlOrZipFile,
    ...
    <div className="bg-white rounded-lg border p-6 space-y-4" data-testid="dc-import-validation-panel">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="px-4 py-2 bg-avisys-blue text-white rounded-lg text-sm disabled:opacity-50"
        >
        ...
        <>
          ...
          {result.acSignoffPreview ? (
            ...
            <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded p-3">
              AC sign-off preview not returned — start <strong>avionics-systems-migration-service</strong> on port 8094 and
