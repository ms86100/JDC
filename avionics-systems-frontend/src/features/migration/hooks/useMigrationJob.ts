...
export function useMigrationJob(options: UseMigrationJobOptions = {}) {
  ...
  const legacyDcImportMutation = useMutation({
    mutationFn: async (params: {
      ...
      const response = await migrationApi.startLegacyDcImport(params);
      return response.data as unknown as MigrationJob;
    },
    onSuccess: (job) => {
      currentJobIdRef.current = job.id;
  ...
  const startImport = useCallback(
    async (
      importType: 'csv' | 'legacy-dc' | 'project',
      params: {
        file?: File;
        sourceProjectId?: string;
        targetProjectId?: string;
      ...
      switch (importType) {
        ...
        case 'legacy-dc':
          ...
          job = await legacyDcImportMutation.mutateAsync({
            file: params.file,
    ...
    [csvImportMutation, legacyDcImportMutation, projectImportMutation]
  );
  ...
  return {
    ...
    legacyDcImport: legacyDcImportMutation,
