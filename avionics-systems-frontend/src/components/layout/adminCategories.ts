/**
 * Administration navigation — Avionics Systems hierarchy (Issues-first).
 * Paths match AdminRoutes.tsx exactly.
 */

import { AVISYS_DC_ADMIN_CATEGORY_ORDER } from './avisysDcNavRegistry';
...
const CATEGORIES_BY_KEY: Record<string, AdminCategory> = {
  issues: {
    ...
    items: [
      ...
      { label: 'Link types', path: '/admin/link-types' },
    ],
  },
  workflows: {
  ...
  projects: {
    ...
    items: [
      ...
      { label: 'Board types', path: '/admin/board-types' },
      { label: 'Quick filter presets', path: '/admin/quick-filters' },
  ...
  users: {
    ...
    items: [
      ...
      { label: 'Notification events', path: '/admin/notification-events' },
  ...
  system: {
    ...
    items: [
      ...
      { label: 'System configuration', path: '/admin/system/config' },
...
export const ADMIN_CATEGORIES: AdminCategory[] = AVISYS_DC_ADMIN_CATEGORY_ORDER.map(
