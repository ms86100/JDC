...
API requests use same-origin `/api/...` paths by default. This keeps one frontend build portable
across localhost, Docker, and Cloud Shell as long as the serving proxy forwards `/api` and `/ws`
to the gateway.
