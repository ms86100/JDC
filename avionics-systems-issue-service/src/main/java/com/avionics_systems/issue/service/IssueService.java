...
import com.avionics_systems.issue.entity.*;
import com.avionics_systems.issue.exception.InvalidTransitionException;
import com.avionics_systems.issue.exception.OptimisticLockException;
import com.avionics_systems.issue.exception.ResourceNotFoundException;
import com.avionics_systems.issue.repository.*;
...
public class IssueService {
    ...
    private final com.avionics_systems.issue.event.IssueEventOutboxPublisher eventOutboxPublisher;
    private final RestTemplate restTemplate;

    @Value("${workflow.service.url}")
    private String workflowServiceUrl;

    @Value("${avionics-systems.workflow.validation-lenient:true}")
    ...
    public IssueService(IssueRepository issueRepository,
                        ...
                        com.avionics_systems.issue.event.IssueEventOutboxPublisher eventOutboxPublisher,
    ...
    public IssueResponse updateIssue(UUID issueId, UpdateIssueRequest request) {
        ...
        List<com.avionics_systems.issue.dto.ChangeItemResponse> changes = new ArrayList<>();

        if (request.getTitle() != null && !request.getTitle().equals(issue.getTitle())) {
            changes.add(com.avionics_systems.issue.dto.ChangeItemResponse.builder().fieldType("avionics-systems").field("summary").oldString(issue.getTitle()).newString(request.getTitle()).build());
            issue.setTitle(request.getTitle());
        }
        if (request.getDescription() != null && !java.util.Objects.equals(request.getDescription(), issue.getDescription())) {
            changes.add(com.avionics_systems.issue.dto.ChangeItemResponse.builder().fieldType("avionics-systems").field("description").oldString(issue.getDescription() != null ? issue.getDescription().substring(0, Math.min(100, issue.getDescription().length())) : null).newString(request.getDescription().substring(0, Math.min(100, request.getDescription().length()))).build());
        ...
        if (request.getAssigneeId() != null && !java.util.Objects.equals(request.getAssigneeId(), issue.getAssigneeId())) {
            changes.add(com.avionics_systems.issue.dto.ChangeItemResponse.builder().fieldType("avionics-systems").field("assignee").oldValue(issue.getAssigneeId() != null ? issue.getAssigneeId().toString() : null).newValue(request.getAssigneeId().toString()).build());
    ...
    public IssueResponse setSecurityLevel(UUID issueId, UUID securityLevelId, UUID userId) {
        ...
        if (userId != null && !securityLevelService.canUserAccessLevel(userId, securityLevelId, issue.getProjectId())) {
            throw new com.avionics_systems.issue.exception.PermissionDeniedException("ASSIGN_ISSUES", "security level " + securityLevelId);
        }
    ...
    public IssueResponse clearSecurityLevel(UUID issueId, UUID userId) {
        ...
        if (userId != null) {
            ...
            if (levelId != null && !securityLevelService.canUserAccessLevel(userId, levelId, issue.getProjectId())) {
                throw new com.avionics_systems.issue.exception.PermissionDeniedException("ASSIGN_ISSUES", "security level " + levelId);
            }
        }
